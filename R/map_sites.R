...
#' @param norm.element chr, element used to normalize data_element 
...
map_sites <- function(s_wuds, data_element, year, state,
                      norm.element=NA, unit_type="county",
                      site_from_to = "from"){
  if (unit_type == "county"){
    # get county polygons
    hc.sub <- subset_county_polygons(year, "STATE_TERR", state)
    hc_subf <- fortify(hc.sub, region = "FIPS")
    ...
    hc_subf <- fortify(hc.sub, region = "HUC8")
    hc.sub@data$id <- hc.sub@data$HUC8
  } else if (unit_type == "aquifer"){
    #get aquifer polygons
  }
  hc_subf <- left_join(hc_subf, hc.sub@data, by = "id")
  ...
  p_elem <- ifelse(is.na(norm.element), data_element,
