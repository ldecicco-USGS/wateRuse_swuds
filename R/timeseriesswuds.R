...
#' @param data_elements character name of data element within available categories by year for state
...
#' @param y_scale allows R to set the y-axis scale given available data range. Defaults to NA which lets R set the scale based on dataset values.
...
time_series_data <- function(s_wuds, dataelements, area.column,
                             plot.points = TRUE,
                             years= NA, areas= NA,
  ...
  if (all(is.na(areas))){
    w_use_sub <- s_wuds[, c("YEAR", area.column, data_elements)]
  } else {
    w_use_sub <-  s_wuds[s_wuds[[area.column]] %in% areas,
                         c("YEAR", area.column, data_elements)]
  }
  if (!any(is.na(years))){
  ...
  df <- w_use_sub[, c("YEAR", area.column, data_elements)]
  ...
  df$data_element <- fix_labs[gsub(pattern = "\\.",
                                  replacement = "-",
                                  x = df$data_element)]
  if (length(unique(df[[area.column]])) > length(c_palette)){
    c_palette_ramp <- colorRampPalette(c_palette)
    c_palette <- c_palette_ramp(length(unique(df[[area.column]])))
  }
