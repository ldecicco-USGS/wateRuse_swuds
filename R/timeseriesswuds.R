...
time_series_data <- function(s_wuds, data.elements, area.column,
  ...
  if (all(is.na(areas))){
    w_use_sub <- s_wuds[, c("YEAR", area.column, data.elements)]
  } else {
  ...
  df <- w_use_sub[, c("YEAR", area.column, data.elements)]
  df <- gather_(df, "data_element", "value", c(data.elements))
  fix.labs <- gsub("\\,", "\\,\n", data.elements$NAME)
  names(fix.labs) <- data.elements$data_element
  df$data_element <- fix.labs[gsub(pattern = "\\.",
                                  replacement = "-",
                                  x = df$data_element)]
